...
void parseData(char *line)
{
  ...
  for (word = strtok(line, " \t\n"); word != NULL;)
  {
    ...
    if (!tmp_num) // 받아온 어절이 문자일 경우
    {
      ...
      { // 알파벳일 경우에만 진행, comment인 경우에는 큰 for문 반복하며 빠져나감
        ...
        { // word가 알파벳임에도 op_table에 없는경우는 label로 등록!
          strcpy(label, word);
          word = strtok(NULL, " \t\n");
          strcpy(opcode, word);
          word = strtok(NULL, " \t\n");
...
void get_info_of_immediate()
{ // immediate.txt의 시작주소, 프로그램 길이, symbol table을 받아온다.
